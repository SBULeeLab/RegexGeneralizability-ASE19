...
class SLRegexDetectorOpinion:
  ...
  canAnalyze: bool
  isVuln: bool
  predictedComplexity: one of PREDICTED_COMPLEXITY_X
    "unknown": Not all detectors support a prediction, or analysis may have failed
    If you have multiple SLRDO's,
    you can determine the worst-case prediction among them using
    the ranking encoded in SLRegexDetectorOpinion.PREDICTED_COMPLEXITY.
  ...
  # Map name to relative cost -- larger is pricier
  PRED_COMPLEXITY_EXP = "exponential"
  PRED_COMPLEXITY_POLY = "polynomial"
  PRED_COMPLEXITY_LINEAR = "linear"
  PRED_COMPLEXITY_UNKNOWN = "unknown"
  PREDICTED_COMPLEXITY = {
    PRED_COMPLEXITY_EXP:      2,
    PRED_COMPLEXITY_POLY:     1,
    PRED_COMPLEXITY_LINEAR:   0,
    PRED_COMPLEXITY_UNKNOWN:  -1
  }
  ...
  def __init__(self):
    ...
    self.isVuln = False
    self.predictedComplexity = SLRegexDetectorOpinion.PRED_COMPLEXITY_UNKNOWN
  ...
  def initFromRaw(self, pattern=None, rawOpinion=None):
    ...
    if rawOpinion['opinion'] == 'TIMEOUT':
      ...
      if self.isVuln:
        # predictedComplexity
        if 'predictedComplexity' in rawOpinion['opinion']:
          predCompl = rawOpinion['opinion']['predictedComplexity']
          if predCompl in SLRegexDetectorOpinion.PREDICTED_COMPLEXITY:
            self.predictedComplexity = predCompl
        else:
          self.predictedComplexity = SLRegexDetectorOpinion.PRED_COMPLEXITY_UNKNOWN
      ...
      elif self.canAnalyze:
        self.predictedComplexity = SLRegexDetectorOpinion.PRED_COMPLEXITY_LINEAR
      else:
        self.predictedComplexity = SLRegexDetectorOpinion.PRED_COMPLEXITY_UNKNOWN
...
class SLRegexAnalysis:
  ...
  DEFAULT_VULN_REGEX_DETECTOR_ROOT = \
    os.path.join(os.environ['REGEX_GENERALIZABILITY_PROJECT_ROOT'], 'measurement-instruments', 'worst-case-performance', 'vuln-regex-detector')
  ...
  def queryDetectors(self, detectors=[], patternVariants=[]):
    """Query detectors. Returns self
    
    Arguments:
      detectors: str[]: the detectors to query
        Empty list: "query all detectors"
      patternVariants: str[]: the variants to try
        Empty list: "try all variants"
    """
    ...
    # Optional argument
    if detectors:
      query["detectors"] = detectors
    if patternVariants:
      query["patternVariants"] = patternVariants
  ...
  def getWorstCasePredictedSpencerPerformanceFromStaticOnly(self):
    predictions = [SLRegexDetectorOpinion.PRED_COMPLEXITY_UNKNOWN]
    for do in self.detectorOpinions: 
      predictions.append(do.predictedComplexity)

    predictions.sort(key=lambda pred: SLRegexDetectorOpinion.PREDICTED_COMPLEXITY[pred])
    return predictions[-1] # Highest-ranked complexity
